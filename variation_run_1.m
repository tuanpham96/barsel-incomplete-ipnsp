...
file_prefix = 'full_var-IP-onoff_var-norminput-onoff_IPsamespeedasSP'; 
max_eta_ip = 1e-3;
...
% specific combinations of initial conditions for with/without norm input
norm_input_conditions = {...
    struct(...
        'train_norm_input', true, ...
        'test_norm_input', true, ...
        'a_init',  5.7, ...
        'b_init', -4.6), ...
    struct(...
        'train_norm_input', false, ...
        'test_norm_input', false, ...
        'a_init',  1, ...
        'b_init', -4.3)};
    
% create varitions
[eta_a_s, eta_b_s, norm_input_conditions] = meshgrid(eta_ip_vec, eta_ip_vec, norm_input_conditions); 

% prepare `norm_input_conditions` for combining with `eta_ip` variables 
norm_input_conditions = structarray_to_struct([norm_input_conditions{:}]); 
norm_input_conditions_fields = fieldnames(norm_input_conditions); 
norm_input_conditions_values = cellfun(@(x) num2cell(to_col_vec(x)), struct2cell(norm_input_conditions), 'uni', 0);
norm_input_conditions_catfieldvals = [norm_input_conditions_fields';norm_input_conditions_values'];
norm_input_conditions_catfieldvals = norm_input_conditions_catfieldvals(:);
    ...
    for j = 1:ncols
        ...
        if i == 1
            ...
            subplot_description = sprintf(...
                ...
                train_subcond_opts.train_norm_input > 0);
