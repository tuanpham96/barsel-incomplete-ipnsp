...
p_inc_train_demo = [0, 0.2, 0.4]; 
fig_prefix = 'demo-vis-p_inc_train'; 
...
for i = 1:length(p_inc_train_demo)
    ...
    for j = 1:num_toplot
        ...
        set(gca, 'xtick', '', 'ytick', '', 'linewidth', 5);
...
sigma_noise = 0.1; 
num_data_per_pinc = 4; 
p_inc_test_demo = [0.4,0.2,0]; 
fig_prefix = 'demo-vis-p_inc_test'; 
...
figure('units', 'normalized', 'position',[0,0,0.6,0.9]);

reordered_ind = reshape(1:size(Us,2), [num_data_per_pinc, length(p_inc_test_demo)])';

colormap('gray');
for i = 1:size(Us,2)
    subplot(3,4,reordered_ind(i)); hold on;
    ...
    if i <= length(p_inc_test_demo)
        ...
        ylabel(panel_description, 'fontsize', 40);
    end
end
...
opts.eta_ip_theta = 1e-3;
opts.eta_ip_a = 1e-2; 
opts.p_sigmoid = 0.2;
opts.subsampled = 500;

opts.points_to_record_W = [3e3,5e4,10e4,15e4,19e4]; 

tic
demo_res = train_with_incomplete_sigmoidthres(opts); 
toc
...
theta_select = demo_res.theta(ind_of_subsampled); 

%% Plot select `W`

fig_prefix = 'demo-vis-training-select-W';
...
for i = 1:num_Ws_toplot    
    ...
    caxis([0, 0.4]);
    daspect([1,1,1]); box on;
    set(gca, 'xtick', '', 'ytick', '', 'linewidth', 10);
...
%% Plot `a`, `theta`, `bar_alpha_W`
fig_prefix = 'demo-vis-training-a-theta-baralpha';
train_transition = ceil(opts.num_train * opts.p_train_complete); 

lgdn_objs = gobjects(1,3); 

figure('WindowStyle', 'normal', 'DefaultLineLineWidth', 5, ...
...
scatter(select_steps, -0.5*ones(length(select_steps),1), 1000, cmap, 'filled');

ylim([-0.5,1.1]);

yyaxis right; 
% smooth for demo 
lgdn_objs(3) = plot(demo_res.t, smoothdata(demo_res.theta, 'movmean',5), '-', 'Color', 0.7*[1,1,1], 'Displayname', '$\theta$'); 
ylim([0.2,1]);
set(gca, 'visible', 'off');
...
fig_prefix = 'demo-vis-training-select-actfun';
x = -0.3:0.01:1.8;
y = arrayfun(@(a,theta) sigmoidal_activation_with_thres(x,a,theta,opts.p_sigmoid), a_select, theta_select, 'uni', 0); 
y = vertcat(y{:});

figure('WindowStyle', 'normal', 'DefaultLineLineWidth', 10, ...
    'Units', 'normalized', 'Position', [0.1,0.1,1,0.6], 'color', 'none'); 
hold on;
colororder(gca, cmap); 
plot(x,y);
set(gca, 'xcolor', 'none', 'ycolor', 'none');
title('activation function', 'fontsize', 100);
