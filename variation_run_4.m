...
file_prefix = 'full_var-sigmoidwiththres-IP-onoff_var-norminput-onoff_IPfasterthanSP'; 
max_eta_ip = 1e-2;
...
def_opts.p_sigmoid = 0.5; 
def_opts.a_init = 10;
def_opts.theta_init = 1; 

def_opts.p_sigmoid = 0.5; 
def_opts.a_init = 10;
def_opts.theta_init = 0.3; 
...
eta_ip_a_vec = [0, max_eta_ip];
eta_ip_theta_vec = [0,max_eta_ip];

% create varitions
[eta_a_s, eta_theta_s] = meshgrid(eta_ip_a_vec, eta_ip_theta_vec); 

training_subconditions = struct(...
    'eta_ip_a', num2cell(eta_a_s(:)), ...
    'eta_ip_theta', num2cell(eta_theta_s(:)));
...
vec_fields_tosave = {'a','theta','bar_alpha_W', 'vec_train_completeness', 'dY_objvsnoise_test'};
...
for i = 1:num_train_subconds
    ...
    parfor j = 1:num_train_p_inc_vec
        ...
        res_j = arrayfun(@(~) train_with_incomplete_sigmoidthres(opts), 1:num_rand, 'uni', 1);
        tmp_res_j = struct; 
...
%% Plot Properties
plt_fields = {'a', 'theta', 'bar_alpha_W', 'vec_train_completeness'}; 
latex_fields = {'a','\theta','\overline{\alpha_W}', '\%c'};
...
var_description = sprintf('[$\\eta_a$ on (%g)/off, $\\eta_{\\theta}$ on (%g)/off]', ...
    max(eta_ip_a_vec), max(eta_ip_theta_vec));
    ...
    for j = 1:ncols
        ...
        if i == 1
            ...
            subplot_description = sprintf(...
                ...
                train_subcond_opts.eta_ip_theta > 0);
...
despline('all');
xlim([0.00,1]*def_opts.num_train);
% 
...
fig_name = sprintf('%s_properties_a0=%.1f_theta0=%.1f', file_prefix,...
    def_opts.a_init, def_opts.theta_init);
export_fig(fullfile('figures/var_4', fig_name), '-r300', '-p0.02'); 
close; 
...
fig_name = sprintf('%s_perfproxy_a0=%.1f_theta0=%.1f', file_prefix,...
    def_opts.a_init, def_opts.theta_init);
export_fig(fullfile('figures/var_4', fig_name), '-r300', '-p0.02'); 
close; 
